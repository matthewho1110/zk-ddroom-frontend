...
function MemberManager({ dataroomId, onSetTitle }) {
  ...
  const dataroomRoleName = {
    "Contract Manager": dataroomData?.dataroom.contractManager,
    Manager: dataroomData?.dataroom.manager,
    "Hidden Manager": dataroomData?.dataroom.hiddenManager,
    Publisher: dataroomData?.dataroom.publisher,
    Reviewer: dataroomData?.dataroom.reviewer,
    Previewer: dataroomData?.dataroom.previewer,
  };
  ...
  const downloadTemplate = () => {
    ...
    data.forEach((row) => {
      Object.keys(row).forEach((key, i) => {
        ...
        maxLengths[i] = Math.max(maxLengths[i] || 0, valueLength);
      });
    });
    ...
    link.download = "Zk-group Batch invite template.xlsx";
  ...
  const handleFileInput = (e) => {
    if (e.target.files.length) {
      ...
      reader.onload = async (event) => {
        ...
        // Collect emails to display in modal
        const emails = sheetData.map((row) => row.email);
        setEmailList(emails);
        setSheetData(sheetData); // Store data for later use
  ...
  const handleConfirm = async () => {
    ...
    try {
      const { data = {}, status } = await axiosInstance.post(
        `${process.env.BACKEND_URI}/datarooms/${dataroomId}/members`,
        {
          invitees: sheetData, // Use the saved sheet data
        }
      );
      const { success = [], failed = [], duplicate = [] } = data;
      if (success.length > 0 && failed.length === 0 && duplicate.length === 0) {
  ...
  //         ) {
  //             setAlert(
  //                 "Invalid file type, please provide an Excel file",
  //                 "error"
  //             );
  //             return;
  //         }
  ...
  //                     ) {
  //                         setAlert(
  //                             "Batch invitation sent successfully",
  //                             "success"
  //                         );
  //                     } else {
  //                         setAlert(
  ...
  //                     }
  //                     if (
  //                         success.findIndex((item) => item === res.email) !=
  //                         -1
  //                     ) {
  //                         return { ...res, status: "success" };
  //                     }
  //                     if (
  //                         duplicate.findIndex((item) => item === res.email) !=
  //                         -1
  //                     ) {
  //                         return { ...res, status: "duplicate" };
  //                     }
  //                     if (
  //                         failed.findIndex((item) => item === res.email) != -1
  //                     ) {
        ...
        <Box display="flex" flexDirection="row" flexWrap="wrap">
          ...
          >
            ...
            <Dialog open={open} onClose={handleCancel}>
              ...
              <DialogContent>
                <DialogContentText>
                  Are you sure you want to invite these people?
                </DialogContentText>
                    ...
                    <TableBody>
                      {emailList.map((email, index) => (
                        <TableRow key={index}>
                          <TableCell
                            style={{
                              border: "1px solid #ccc", // Adds border around the cell
                              padding: "8px", // Adds some padding to the cell
                            }}
                          >
                            {email}
                          </TableCell>
