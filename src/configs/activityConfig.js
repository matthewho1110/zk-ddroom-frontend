...
module.exports.EVENT_TYPES = {
    ...
    Dataroom_update: {
        ...
        description: (activity) => {
            ...
            const oldDataroomMaxStorage =
                oldDataroomParsed?.maxStorage == -1
                    ? "Unlimited"
                    : oldDataroomParsed?.maxStorage / 1073741824 + "GB";
            const newDataroomMaxStorage =
                newDataroomParsed?.maxStorage == -1
                    ? "Unlimited"
                    : newDataroomParsed?.maxStorage / 1073741824 + "GB";
            ...
            return (
                <div className="overflow-x-div">
                    ...
                    <br></br>
                    <t onClick={onClick} className="show-hide-btn">
                        {showDetails ? (
                            <t>hide details</t>
                        ) : (
                            <t>show details</t>
                    ...
                    {showDetails ? (
                        <div>
                            <t>
                                Name: {oldDataroomName} → {newDataroomName} /{" "}
                            </t>
                            <t>
                                Description: {oldDataroomDescription} →{" "}
                                {newDataroomDescription} /{" "}
                            </t>
                            <t>
                                Organization: {oldDataroomOrganization} →{" "}
                                {newDataroomOrganization} /{" "}
                            </t>
                            <t>
                                Phase: {oldDataroomPhase} → {newDataroomPhase} /{" "}
                            </t>
                            <t>
                                Max Storage: {oldDataroomMaxStorage} →{" "}
                                {newDataroomMaxStorage}
                            </t>
                        </div>
                    ) : null}
                </div>
    ...
    UserGroup_update: {
        ...
        description: (activity) => {
            const oldGroupName =
                activity?.details?.oldGroupName || "Unknown group";
            const newGroupName =
                activity?.details?.newGroupName || "Unknown group";
                ...
                <div className="overflow-x-div">
                    <div>
                        ...
                        <a
                            ...
                            href={
                                `/dataroom/${activity.dataroomId}/files?filePath=` +
                                (activity.details?.directory?._id || directory)
                            }
                            className="link-btn"
                        >
                            {activity.details?.directory?.path || directory}
                        </a>
                        {files.length > 1 ? (
                            <div>
                                <t onClick={onClick} className="show-hide-btn">
                ...
                <div>
                    {deletedFiles?.map((file) => (
                        <>
                            ...
                            {file != deletedFiles[deletedFiles.length - 1] ? (
                                <t> , </t>
                            ) : (
                                <t></t>
                            )}{" "}
            ...
            return (
                <div className="overflow-x-div">
                    <div>
                        ...
                        <br></br>
                        <t onClick={onClick} className="show-hide-btn">
                            {showLinks ? <t>hide files</t> : <t>show files</t>}
                        </t>
                        {showLinks ? <Links /> : null}
                    </div>
                </div>
                ...
                <div>
                    {movedFiles?.map((file) => (
                        <>
                            ...
                            {file != movedFiles[movedFiles.length - 1] ? (
                                <t> , </t>
                            ) : (
                                <t></t>
                            )}{" "}
            ...
            return (
                <div className="overflow-x-div">
                    <div>
                        <t>
                            {movedFiles.length == 1 ? (
                                <t> File is moved from </t>
                            ) : (
                                <t> Files are moved from </t>
                            )}
                            <a
                                target="_blank"
                                href={
        ...
        description: (activity) => {
            ...
            const info =
                (profile?.data?.firstname
                    ...
                    : "Unregistered") +
                " " +
                (profile?.data?.lastname ? profile.data.lastname : "User") +
                " (" +
                email +
                ") ";
    ...
    QA_create_a_question: {
        ...
        description: (activity) => {
            ...
            const navigate = () =>
                openQa.push(
                    `/dataroom/${activity.dataroomId}/qa/detail?qid=${questionId}`
                );
            return (
                <div>
                    <t>A new question - </t>
                    <u className="link-btn" onClick={navigate}>
    ...
    QA_update_a_question: {
        ...
        description: (activity) => {
            ...
            const oldQuestionTags =
                oldQuestionParsed?.tags.length == 0
                    ? "No Tags"
                    : oldQuestionParsed?.tags.map((tag) => tag.name).join(" ");
            const oldQuestionExperts =
                oldQuestionParsed?.experts.length == 0
                    ? "No Experts"
                    : oldQuestionParsed?.experts
                          .map(
                              (expert) =>
                                  expert.firstname + " " + expert.lastname
                          )
                          .join(", ");
            const oldQuestionDescription = oldQuestionParsed?.description.slice(
                3,
                -4
            );
            const updatedQuestionTitle = updatedQuestionParsed?.title;
            const updatedQuestionTags =
                updatedQuestionParsed?.tags.length == 0
                    ? "No Tags"
                    : updatedQuestionParsed?.tags
                          .map((tag) => tag.name)
                          .join(" ");
            const updatedQuestionExperts =
                updatedQuestionParsed?.experts.length == 0
                    ? "No Experts"
                    : updatedQuestionParsed?.experts
                          .map(
                              (expert) =>
                                  expert.firstname + " " + expert.lastname
                          )
                          .join(", ");
            const updatedQuestionDescription =
                updatedQuestionParsed?.description.slice(3, -4);
            ...
            return (
                <div className="overflow-x-div">
                    <u className="link-btn" onClick={navigate}>
                        {oldQuestionTitle || "Unknown Question"}
                    </u>
                    <t> is updated</t>
                    <br></br>
                    <t onClick={onClick} className="show-hide-btn">
                    ...
                    {showDetails ? (
                        <div>
                            <t>
                                Title: {oldQuestionTitle} →{" "}
                                {updatedQuestionTitle} /{" "}
                            </t>
                            <t>
                                Tags: {oldQuestionTags} → {updatedQuestionTags}{" "}
                                /{" "}
                            </t>
                            <t>
                                Experts: {oldQuestionExperts} →{" "}
                                {updatedQuestionExperts}
                            </t>
                            <t>
                                Description: {oldQuestionDescription} →{" "}
