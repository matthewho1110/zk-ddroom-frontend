...
function MemberManager({ dataroomId, onSetTitle }) {
    ...
    const handleRemoveMember = async (memberId) => {
        try {
            await axiosInstance.delete(
                `datarooms/${dataroomId}/members/${memberId}`
            );
    ...
    const handleMemberAction = (memberId, type) => {
        if (type === "Remove_User") {
            ...
            const memberToReport = members.find(
                (member) => member._id === memberId
            );
    ...
    const resendInvitation = async (memberId) => {
        try {
            ...
            console.log(err);
            setAlert(
                "Encountered an error while resending invitation",
                "error"
            );
        }
    };
    ...
    const downloadTemplate = () => {
        ...
        link.download = "DDRoom Batch invite template.xlsx";
    ...
    // Manage group form
    const closeEditGroupForm = () =>
        setEditGroupFormMode(EDIT_FORM_MODES.closed);
        ...
        return (
            ...
            >
                ...
                >
                    <EditGroupForm
                        group={
                            editGroupFormMode == EDIT_FORM_MODES.edit &&
                            currentGroup
                        }
                        dataroomId={dataroomId}
